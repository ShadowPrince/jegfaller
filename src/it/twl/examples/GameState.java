...
public class GameState extends BasicTWLGameState {
	...
	private void createButton(RootPane rp) {
		...
		button.addCallback(new Runnable() {
			public void run() {
				System.out.println("some action!");
                FileSelector sel = new FileSelector();
	...
	private void createHelloPanel(RootPane rp) {
		helloPanel = new BoxLayout(Direction.HORIZONTAL);
		helloPanel.setTheme("panel");

		Label label = new Label();
		label.setText("Hello world Panel asfas f asfaskj faskldjf lkasjdlkf jalsdjflk j");

        Button b = new Button();
        b.setText("kubar");
        EditField t = new EditField();
        t.setText("kubar");
        t.setMinSize(100, 50);
